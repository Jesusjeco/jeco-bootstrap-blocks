...
class Create_Block_Command
{
  ...
  {
    $block_path = JECO_BOOTSTRAP_BLOCKS_ROOT_PATH . "blocks/$block_name";
  ...
  {
    $template_path = JECO_BOOTSTRAP_BLOCKS_ROOT_PATH . 'templates/render.php';
    if (file_exists($template_path)) {
      ...
      file_put_contents(JECO_BOOTSTRAP_BLOCKS_ROOT_PATH . $render_template, $render_content);
    } else {
  ...
  {
    $register_file = JECO_BOOTSTRAP_BLOCKS_INC_PATH . 'register-blocks.php';
  ...
  private function generateBlockRegistrationCode($block_name, $render_template)
  {
    return "
        ...
        acf_register_block_type([
            ...
            'render_template'   => JECO_BOOTSTRAP_BLOCKS_ROOT_PATH . '$render_template',
            'category'          => 'formatting',
  ...
  private function createStyleFile($block_name, $style_file)
  {
    ...
    file_put_contents(JECO_BOOTSTRAP_BLOCKS_ROOT_PATH . $style_file, $template_for_style);
  }

  private function registerStyle($block_name)
  {
    $register_file = JECO_BOOTSTRAP_BLOCKS_INC_PATH . 'register-styles.php';
    if (!file_exists($register_file) || !is_writable($register_file)) {
  ...
  {
    $register_file = JECO_BOOTSTRAP_BLOCKS_INC_PATH . 'register-editor-styles.php';
    if (!file_exists($register_file) || !is_writable($register_file)) {
  ...
  private function generateStyleRegistrationCode($block_name)
  {
    return <<<CODE
        ...
        if (has_block('acf/$block_name')) {
            wp_enqueue_style(
              '$block_name-block',
              JECO_BOOTSTRAP_BLOCKS_ROOT_URL . 'blocks/$block_name/style.css',
              array()
            );
        }\n
        CODE;
    ...
    return <<<CODE
      ...
      if (has_block('acf/$block_name')) {
        wp_enqueue_style(
          ...
          JECO_BOOTSTRAP_BLOCKS_ROOT_URL . 'blocks/$block_name/editor-style.css',
          array()
        );
      }
    CODE;
